    ...
    myData().then((data) => {
        ...
        for (let i = 0; i < detail['article_set'].length; i++) {
            ...
            <div class="my_article" id="mypage-box">
                <b id="title"><a href="${frontend_base_url}/templates/article_detail.html?=${article_id}"> ${title}</b><span> | ${exposure_start_date}</span>
                <!--얘는 삭제예정-->
                <div class="image_frame">
                <a href="${frontend_base_url}/templates/article_detail.html?=${article_id}" class="image fit"><img src="../static${image}" alt="" /></a>
                </div>
            </div>
        </div>
        </div>`
